...
( cd sdcard/boot ; 
echo "setenv bootargs console=tty0 root=/dev/mmcblk0p2 rootwait consoleblank=0 panic=10 drm_kms_helper.drm_leak_fbdev_smem=1
ext2load mmc 0 0x41000000 openvario.dtb
ext2load mmc 0 0x64000000 initrd.img-$LINUX_VERSION
ext2load mmc 0 0x42000000 vmlinuz-$LINUX_VERSION
bootz 0x42000000 0x64000000:0xA00000 0x41000000" |$SUDO tee boot.cmd || exit 1
